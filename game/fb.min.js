...
var deathAnimation = function () {
  if (splash) {
    ...
    splash = undefined;
  } else ctx.drawImage(scoreBoard, width / 2 - 118, height / 2 - 54);
...
var drawSky = function () {
  var e = 0;
  while (e < width) {
    ctx.drawImage(sky, e, height - 221);
    e += sky.width;
  }
};
var drawLand = function () {
  var e = -dist;
  while (e < width) {
    ctx.drawImage(land, e, height - 112);
    e += land.width;
  }
  dist = dist + 2;
  var t = Math.floor(dist - width * 0.65) % 220;
  if (dist >= width * 0.65 && Math.abs(t) <= 1) {
    score++;
  }
};
var drawPipe = function (e, t) {
  ctx.drawImage(pipe, e, 0, pipe.width, t);
  ctx.drawImage(pipeDown, e, t);
  ctx.drawImage(pipe, e, t + 168 + delta, pipe.width, height - 112);
  ctx.drawImage(pipeUp, e, t + 144 + delta);
  if (
    e < birdPos + 34 &&
    e + 52 > birdPos &&
    (birdY < t + 24 || birdY + 24 > t + 144 + delta)
  ) {
    clearInterval(animation);
    death = 1;
  } else if (e + 40 < 0) {
...
var drawScore = function () {
  ...
  ctx.fillStyle = "#000";
  var e = "" + score;
  ctx.strokeText(e, (width - ctx.measureText(e).width) / 2, height * 0.15);
  ctx.fillText(e, (width - ctx.measureText(e).width) / 2, height * 0.15);
};
var drawCanvas = function () {
  clearCanvas();
  drawSky();
  for (var e = pipeSt; e < pipeNumber; ++e) {
    drawPipe(width - dist + e * 220, pipes[e]);
    if (mode == 2) {
      if (pipesDir[e]) {
        if (pipes[e] + 1 > height - 300) {
          pipesDir[e] = !pipesDir[e];
          pipes[e] -= 1;
        } else pipes[e] += 1;
      } else {
        if (pipes[e] - 1 < 10) {
...
var anim = function () {
  animation = setInterval(drawCanvas, 1e3 / 60);
};
var jump = function () {
  if (death) {
    dist = 0;
